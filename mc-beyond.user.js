...
( function() {
    ...
    var isNarrow = location.host.match(/^(?:[m|y]\d+\.bmcaxis)\.jp/i) ? true : false;
    ...
    if (isNarrow && OPT_VILLAGE_LIST_BOX) disp_villageListBox();		// ◯ 拠点リスト常時表示
    if (OPT_VILLAGE)     disp_village();								// ◯ 施設建設可否・建設中・破棄中表示
    if (OPT_BASELINK)    disp_baseLink();								// ◯ 拠点リストに内政・援軍等リンク追加
    if (OPT_MEMO)        disp_memo();									// ◯ メモ機能
    if (OPT_ALLY)        disp_AllianceInfo(); 							// △ 同盟表示強化									同盟員情報CSV のみ未動作 とりあえず放置
//  if (OPT_BOOKMARK)    disp_bookmark();								// × ブックマーク									大幅に手直し必要
    if (OPT_XYLINK)      disp_XYLink();									// ◯ 座標リンク
    if (OPT_TTBL)        disp_TTable();									// ◯ 戦争ログ整形
    if (OPT_DETAILS)     disp_Details();								// △ 戦争ログ合計表示								多分大丈夫だけど未テスト
//  if (OPT_DECK)        disp_Deck();									// × デッキ画面改善								もともと未使用
    if (OPT_CTIME_B)     disp_CompleteTimeBuild();						// ◯ 建設時：完了日時表示
    if (OPT_CTIME_U)     disp_CompleteTimeUnit();						// ◯ 兵作成：完了日時表示
//  if (OPT_RES_T)       disp_ResourcesTotal();							// × 資源生産量合計表示							表示場所がない
    if (OPT_RES_TIME)    disp_ResourcesTime();							// ◯ 倉庫溢れ or 建設可能までの資源生産時間表示	リアルタイムではない
    if (OPT_REMOVELIST)  disp_RemoveList();								// ◯ 建設・破棄一覧
    if (OPT_MAPLIST)     disp_MapList();								// ◯ 地形一覧表示									ただし mc_npc_castle_info と競合する
    if (OPT_USER_STAR)   disp_UserStar();								// ◯ プロフィール画面領地情報表示
    if (OPT_USER_LEVEL)  disp_UserLevel();								// ◯ プロフィール画面領地レベル表示
    if (OPT_MAPCENTER)   disp_MapCenter();								// ◯ マップ中央表示
    if (OPT_DELMSG)      disp_DeleteMessages();							// ◯ 書簡・報告書削除ボタン
    if (OPT_TSENDTIME)   disp_TSendTime();								// ◯ 出発時間計算
//  if (OPT_SMALLBTN)    disp_SmallButton();							// × 小さいボタン									必要無さそうなので無効化
    if (OPT_ATTACKMAP)   disp_AttackMap();								// ◯ 出兵状況マップ表示
    if (OPT_CARD_CMB)    disp_CardCombine();							// ◯ 合成支援ボタン
//  if (OPT_YOROZU)      disp_Yorozu();									// × ヨロズ引き忘れ防止							ヨロズないんや
    if (OPT_HPREST)      disp_HPRestTime();								// × ＨＰ回復時間表示								そのうち直す
    if (OPT_TBREST)      disp_ToubatsuRestTime();						// × 討伐回復時間表示								そのうち直す
    if (OPT_MAPHELP)     installMapXYHelper();							// × 座標入力支援									いらなくね？
    if (OPT_TRDHELP)     installTradeHelper();							// ◯ トレード入力支援
    if (OPT_SEISAN)      disp_Seisan();									// ◯ 拠点生産量表示
    if (OPT_NEXT_FAME)   disp_nextFameTimer();							// ◯ 名声タイマー
//  if (OPT_TIMER_DEPOT) disp_timerDepot();								// × タイマー格納									いらなくね？
    if (OPT_LOGBOX)      disp_logMessageBox();							// ◯ ログ表示
    if (OPT_TTALLYPRSN)  disp_ToolTipsAllyPerson();						// ◯ 同盟・君主表示ツールチップ表示
    if (OPT_TTDISTANCE)  disp_ToolTipsDistance();						// △ 飛行場・航空基地がないためテストできない
    ...
    function initResources() {
		...
		var tt = document.evaluate('//div[@class="status_rsrc_data"]/p[2]',document, null, XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE, null);
		RES_GROW_W['wood'] = parseInt(tt.snapshotItem(0).innerHTML.match(/(\d+)/), 10);
		RES_GROW_B['wood'] = parseInt(tt.snapshotItem(0).innerHTML.match(/\+(\d+)</), 10);
/*
        ...
        for (var i = 0; i < spns.length; i++) {
            var str = spns[i].previousSibling.nodeValue.match(/(鉄|石油|アルミ|食)\s+(-?\d+)/);
            if (str) {
                var name = '';
                switch (str[1]) {
                    case '鉄': name = 'wood'; break;
                    case '石油': name = 'stone'; break;
                    case 'アルミ': name = 'iron'; break;
                    case '食': name = 'rice'; break;
                }
...
//      OPT_DECK = cloadData('OPT_DECK', 1);
        ...
        OPT_TTDISTANCE_ITEMS = cloadData('OPT_DISTANCE_ITEMS', '["軽戦車(7)", "水雷艇(5)", "軽戦闘機(12)", "重戦車(10)", "駆逐艦(8)", "重戦闘機(15)", "工兵(3)", "自走臼砲(6)"]', true, true);
        ...
        OPT_LOGBOX = cloadData('OPT_LOGBOX', 1);
...
//      csaveData('OPT_DECK', OPT_DECK);
    ...
    function getMyInfo() {
        if ((location.pathname == '/user/' || location.pathname == '/user/index.php') && !URL_PARAM.user_id) {
...
//      ccreateCheckBox(td1, 'OPT_DECK', OPT_DECK, 'デッキ画面改善', 'デッキ画面の改善を行います', 0);
        ...
        ccreateTextBox(td3, 'OPT_LOG_EXP_TIME', OPT_LOG_EXP_TIME, '時間(分)', 'ログの保存期間を分数で指定します（初期値：180）', 4, 34);
    ...
    function createDistanceBox(container, items, num) {
        var sels = ['その他', '無し', '新兵(6)',
                    '軽戦車(7)', '水雷艇(5)', '軽戦闘機(12)', '工作員(9)', '工兵(3)',
                    '重戦車(10)', '駆逐艦(8)', '重戦闘機(15)', '特殊部隊(20)', '自走臼砲(6)'];
        for (var i = 0; i < num; i++) {
            var src = '';
            ...
            tb.title = '例)「高速重戦車ユニット(50)」のように、半角カッコ内に移速を記入して下さい';
            if (src == '') {
                tb.value = items[i];
            } else {
    ...
    function disp_village() {
        ...
        var costs = [];
        costs['製鉄所'] = cost_wood;
        costs['製油所'] = cost_stone;
        costs['アルミニウム精錬所'] = cost_iron;
        costs['食品加工工場'] = cost_rice;
        costs['歩兵学校'] = cost_renpei;
        costs['陸軍士官学校'] = cost_heisya;
        costs['海軍兵学校'] = cost_yumi;
        costs['空軍士官学校'] = cost_uma;
        costs['工兵学校'] = cost_heiki;
        costs['兵舎'] = cost_syukusya;
        costs['駐屯地'] = cost_daisyukusya;
        costs['飛行場'] = cost_kunren;
        costs['航空基地'] = cost_enseikunren;
        costs['武装研究所'] = cost_kajiba;
        ...
        costs['銅像'] = cost_doujaku;
//      costs['修行所'] = cost_syuugyousyo;
        costs['総司令部'] = cost_shiro;
        costs['前線基地'] = cost_toride;
        costs['補給基地'] = cost_mura;
        ...
        for (var i = 0; i < bookmarks; i++) {
            ...
            if (tmp != '') {
                ...
                if (data.length == 3) {
                    ...
                    var link1 = d.createElement('a');
                    link1.href = caddSessionId('/user/?user_id=' + data[0]);
                    link1.title = data[3];
...
//           
            ...
            reg = /新兵\s+軽戦車\s+水雷艇\s+軽戦闘機\s+重戦車\s+駆逐艦\s+重戦闘機\s+工作員\s+特殊部隊\s+工兵\s+自走臼砲\s+ユニット.*(\n兵士\s+|\n)(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(.+)/g;
            tmpHTML = tmpHTML.replace(reg,
                function() {
                    var txt = '<table class="tables"><tr><th class="solClass">　</th>' +
    ...
    function disp_Details() {
        ...
        var tr = d.createElement('tr');
        var thtxt = ['新兵', '軽戦車', '水雷艇', '軽戦闘機', '重戦車', '駆逐艦', '重戦闘機', '工作員', '特殊部隊', '工兵', '自走臼砲', 'ユニット'];
        var th = d.createElement('th');
        th.className = 'blank';
        tr.appendChild(th);
        for (var i = 0; i < 12; i++) {
    ...
    function disp_CompleteTimeBuild() {
        ...
        for (var i = 0; i < tds.length; i++) {
            var td = tds[i];
            var th = $s('.//th[contains(text(), "建設に必要な食料消費量") or contains(text(), "建設に必要な資材")]', td.parentNode.parentNode);
            if (th) {
...
//console.log("Start ResourcesTimer()");
...
//console.log("== END ResourcesTimer() ==");
        }
    ...
    function disp_ToolTipsDistance() {
        ...
        function showToolTips(evt, distance, trainingLevel, eTrainingLevel) {
            ...
            var dv = d.createElement('div');
            dv.innerHTML = '移動時間/到着時刻の目安　　　　距離 [' + distance.toFixed(2) + ']<br>飛行場Lv.' + trainingLevel + ' (+' + (TL*5) + '%)<br>航空基地Lv.'+eTrainingLevel+' (+'+(eTrainingLevel*0.1*distance).toFixed(3)+'%)' ;
            tw.appendChild(dv);
    ...
    function disp_UserLevel() {
        ...
        function showProfile() {
            //tableLevel欄追加
            cappendColumnForProfile('レベル', 'beyond_level');
    ...
    function disp_SmallButton() {
        ...
        if (!isNarrow) {
            ...
            btnConfs = {
                ...
                hbtn_special_red : {width : 26, start : -72},
                hbtn_yorodudas : {width : 25, start : -2},
                hbtn_bushoduel : {width : 27, start : -1},
                hbtn_busyobook : {width : 20, start : -6}
            };
...
//      var tim = timetxt.match(/^(\d+):(\d+):(\d+)/);				// 2013.01.31
    ...
    function Suzan_Seisan(inner) {
        ...
        var areas = $x('//area[contains(@alt, "食") or contains(@alt, "田") or contains(@alt, "ボ") or contains(@alt, "鉄") or contains(@alt, "油") or contains(@alt, "岩") or contains(@alt, "製") or contains(@alt, "ア")]');
        ...
        var text = '<ul>';
        text += '<li><img align="middle" alt="鉄" src="'     + IMG_DIR +  'common/ico_wood2.gif"> 鉄　　　' + seisan.w + '</li>';
        text += '<li><img align="middle" alt="石油" src="'   + IMG_DIR + 'common/ico_stone2.gif"> 石油	　' + seisan.s + '</li>';
        text += '<li><img align="middle" alt="アルミ" src="' + IMG_DIR +  'common/ico_iron2.gif"> アルミ　' + seisan.i + '</li>';
        text += '<li><img align="middle" alt="食" src="'     + IMG_DIR +  'common/ico_lice2.gif"> 食　　　' + seisan.r + '</li>';
    ...
    function disp_nextFameTimer() {
        ...
        function createTimerText(date) {
            return document.createTextNode('('+date.toLocaleTimeString().replace(/^00/, 0)+')');
        }
